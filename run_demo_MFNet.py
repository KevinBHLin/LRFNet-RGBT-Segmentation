...
#############################################################################################
parser.add_argument('--model_name', '-m', type=str, default='LRFNet')
parser.add_argument('--weight_name', '-w', type=str, default='')
parser.add_argument('--file_name', '-f', type=str, default='checkpoint_mfnet.pth.tar')
...
parser.add_argument('--data_dir', '-dr', type=str, default='data/MFdataset')
parser.add_argument('--model_dir', '-wd', type=str, default='checkpoint/')
...
if __name__ == '__main__':
    ...
    model = eval(args.model_name)(n_class=args.n_class,num_resnet_layers=18)
    #model = model.cuda()
    ...
    ave_time_cost = 0.0
    #labelprocess = Gaussian_mixture12()
    model.eval()
    with torch.no_grad():
        for it, (images, labels, names) in enumerate(test_loader):
            ...
            logits = model(images,istrain=False)  # logits.size(): mini_batch*num_class*480*640
            end_time = time.time()
            ...
            conf_total += conf
            # save demo images
            visualize(image_name=names, predictions=logits.argmax(1), model_name=args.model_name)
